...
def create_comparative_heatmaps_gl(results_dir, experiment_dirs, metric_name="Price", figsize=(12, 8)):
    ...
    # Extract data for both cases
    gamma_noref, lambda_noref, metric_noref = extract_metric_data_gl(experiment_dirs['noreference'], metric_name)
    gamma_ref, lambda_ref, metric_ref = extract_metric_data_gl(experiment_dirs['reference'], metric_name)

    # Convert to numpy arrays
    gamma_noref, lambda_noref, metric_noref = np.array(gamma_noref), np.array(lambda_noref), np.array(metric_noref)
    ...
    for i, (gamma_vals, lambda_vals, metric_vals, title, ax, cmap, norm) in enumerate([
        (gamma_noref, lambda_noref, metric_noref, f"{metric_name} (No Reference)", ax1, cmap, mcolors.Normalize(vmin=vmin, vmax=vmax)),
        (gamma_ref, lambda_ref, metric_ref, f"{metric_name} (Reference)", ax2, cmap, mcolors.Normalize(vmin=vmin, vmax=vmax)),
        (gamma_ref, lambda_ref, metric_diff, f"{metric_name} Difference (Ref - No Ref)", ax3, "coolwarm", mcolors.TwoSlopeNorm(vmin=vmin_diff - 0.01, vcenter=0, vmax=vmax_diff + 0.01))
    ]):
        ...
        # Add contour lines for better readability
        ax.contour(data_grid, levels=6, colors="black", linewidths=0.5, extent=[min(unique_lambdas), max(unique_lambdas), min(unique_gammas), max(unique_gammas)])
