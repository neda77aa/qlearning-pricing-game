...
from input.visualization import create_comparative_heatmaps, create_single_heatmap, create_single_heatmap_gamma_only, create_comparative_heatmaps_gl, create_single_heatmap_gl, create_single_heatmap_lossaversion, create_comparative_heatmaps_miss 
...
if __name__ == '__main__':
    ...
    if Desired_Experiment == 'alpha_beta':
        ...
        for demand_type in ['reference']:
            for common_reference in [True,False]:
                ...
                game = model(n=2, k = 15, memory = 1,alpha=0.0075, beta=0.01/25000, num_sessions = num_sessions, aprint = aprint, demand_type = demand_type, common_reference = common_reference)
    ...
    if Desired_Experiment == 'loss_aversion':
       # Define parameter ranges to test
        ...
        # Generate heatmaps
        fig_profit = create_single_heatmap_lossaversion("../Results_sockeye/experiments",  experiment_name=experiment_name, metric_name="Profit")
        fig_price_gain = create_single_heatmap_lossaversion("../Results_sockeye/experiments", experiment_name=experiment_name, metric_name="Price Gain")
        fig_price = create_single_heatmap_lossaversion("../Results_sockeye/experiments", experiment_name=experiment_name, metric_name="Price")
        fig_cycle = create_single_heatmap_lossaversion("../Results_sockeye/experiments", experiment_name=experiment_name, metric_name="Cycle Length")
        fig_foc = create_single_heatmap_lossaversion("../Results_sockeye/experiments", experiment_name=experiment_name, metric_name="FOC")
        # Create "Figures" directory
        figures_dir = os.path.join("../Results_sockeye/experiments", experiment_name, "Figures")
        os.makedirs(figures_dir, exist_ok=True)
